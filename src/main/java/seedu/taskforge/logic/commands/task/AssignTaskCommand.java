...
public class AssignTaskCommand extends TaskCommand {
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        List<PersonTask> resolvedTasksToAssign = resolveTasksToAssign(assignTaskDescriptor, personToEdit, model);
    ...
    private static List<PersonTask> resolveTasksToAssign(AssignTaskDescriptor descriptor, Person person, Model model)
        ...
        if (descriptor.getTaskIndexes().isPresent()) {
            List<Task> indexedTasks = resolveTasksByIndex(descriptor.getTaskIndexes().orElseThrow(), person, model);
            return resolveTasksWithProjectTracking(indexedTasks, person, model);
        }
    ...
    private static List<Task> getAssignableTasks(Person person, Model model) {
        List<PersonProject> assignedProjects = person.getProjects();
        List<Project> allProjects = model.getProjectList();

        List<Task> assignableTasks = new ArrayList<>();
        for (PersonProject assignedProject : assignedProjects) {
            int projectIndex = assignedProject.getProjectIndex();
            if (projectIndex >= 0 && projectIndex < allProjects.size()) {
                Project project = allProjects.get(projectIndex);
                project.getTasks().forEach(task -> assignableTasks.add(new Task(task.description, project.title)));
            }
        }
